...
We can see that there's a pretty clear relationship between these two variables. But we'd like to go beyond this to quantify the relationship statistically.
...
The output of the `summary` function contains important information about:
  ...
  3. `Pr(>|t|)`: Results of a t-test to determine if the intercept or parameters' slopes are different from 0
  4. `Adjusted R squared`: How well does the model explain the data?
  5. `F-statistic`(ANOVA): Is the model significantly different from a model with no predictor (null model)?
